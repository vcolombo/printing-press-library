...
In local mode: searches locally synced data only.`,
  ...
  makerworld-pp-cli search "error timeout"

  # Force local search only
  makerworld-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  makerworld-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  makerworld-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'makerworld-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "designers":
				results, err = db.SearchDesigners(query, limit)
			case "designs":
				results, err = db.SearchDesigns(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchDesigners(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search designers failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchDesigns(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search designs failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
