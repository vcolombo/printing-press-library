...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  makerworld-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  makerworld-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"categories", "designs", "designs-nav", "designs-youlike"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  makerworld-pp-cli workflow status

  # Show status as JSON
  makerworld-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("makerworld-pp-cli")
			}
