...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "makerworld-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MAKERWORLD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MAKERWORLD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "makerworld-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/makerworld-pp-cli/feedback.jsonl.
When ` + "`MAKERWORLD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MAKERWORLD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  makerworld-pp-cli feedback list
  makerworld-pp-cli feedback list --limit 5
  makerworld-pp-cli feedback list --json`,
