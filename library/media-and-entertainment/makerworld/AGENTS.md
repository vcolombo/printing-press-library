# MakerWorld Printed CLI Agent Guide

This directory is a generated `makerworld-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
makerworld-pp-cli doctor --json
makerworld-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
makerworld-pp-cli which "<capability>" --json
makerworld-pp-cli <command> --help
```
...
```bash
makerworld-pp-cli <command> --agent
```
...
```bash
makerworld-pp-cli <command> --help
makerworld-pp-cli <command> --dry-run --agent
```
