...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "pixabay-pp-cli",
		Short: `Pixabay CLI — Every Pixabay search filter the API has — plus a 24-hour local cache, offline search, resumable downloads with built-in attributi…`,
		Long: `Pixabay CLI — Every Pixabay search filter the API has — plus a 24-hour local cache, offline search, resumable downloads with built-in attributi…
...
Health check: run 'pixabay-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("pixabay-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'pixabay-pp-cli profile list')")
	...
	// Pixabay API Terms of Service: credit Pixabay with a visible link whenever
	// search results are displayed. emitPixabayCredit writes to stderr for
	// human/TTY runs only, so machine output on stdout stays clean.
	rootCmd.PersistentPostRunE = func(cmd *cobra.Command, args []string) error {
		emitPixabayCredit(cmd, flags)
		return nil
	}
	...
	rootCmd.AddCommand(newNovelContributorsCmd(flags))
	rootCmd.AddCommand(newNovelMediaCmd(flags))
