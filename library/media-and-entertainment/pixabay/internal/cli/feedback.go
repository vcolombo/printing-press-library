...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PIXABAY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PIXABAY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pixabay-pp-cli/feedback")
...
When ` + "`PIXABAY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PIXABAY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pixabay-pp-cli feedback list
  pixabay-pp-cli feedback list --limit 5
  pixabay-pp-cli feedback list --json`,
