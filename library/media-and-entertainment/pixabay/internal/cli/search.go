...
In local mode: searches locally synced data only.`,
  ...
  pixabay-pp-cli search "error timeout"

  # Force local search only
  pixabay-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  pixabay-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  pixabay-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /api/
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/api/", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'pixabay-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
