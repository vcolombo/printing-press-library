...
type Config struct {
	...
	PixabayApiKey    string          `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("PIXABAY_API_KEY"); v != "" {
		cfg.PixabayApiKey = v
		cfg.markEnvOverride("PixabayApiKey")
		cfg.AuthSource = "env:PIXABAY_API_KEY"
		cfg.CredentialSource = "env:PIXABAY_API_KEY"
	}
	...
	if v := os.Getenv("PIXABAY_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("PIXABAY_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "pixabay-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	replacements := map[string]string{
		"token":           token,
		"api_key":         c.PixabayApiKey,
		"PIXABAY_API_KEY": c.PixabayApiKey,
	}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.PixabayApiKey = creds.PixabayApiKey
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.PixabayApiKey = token
	delete(c.envOverrides, "PixabayApiKey")
	c.updateFileConfigField("PixabayApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["PixabayApiKey"] {
			out.PixabayApiKey = c.fileConfig.PixabayApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "PixabayApiKey":
		c.fileConfig.PixabayApiKey = c.PixabayApiKey
	}
}
