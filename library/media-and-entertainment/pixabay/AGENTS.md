# Pixabay Printed CLI Agent Guide

This directory is a generated `pixabay-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
pixabay-pp-cli doctor --json
pixabay-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
pixabay-pp-cli which "<capability>" --json
pixabay-pp-cli <command> --help
```
...
```bash
pixabay-pp-cli <command> --help
pixabay-pp-cli <command> --dry-run --agent
```
