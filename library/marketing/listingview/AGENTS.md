# ListingView Printed CLI Agent Guide

This directory is a generated `listingview-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
listingview-pp-cli doctor --json
listingview-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
listingview-pp-cli which "<capability>" --json
listingview-pp-cli <command> --help
```
...
```bash
listingview-pp-cli <command> --agent
```
...
```bash
listingview-pp-cli <command> --help
listingview-pp-cli <command> --dry-run --agent
```
