...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"LISTINGVIEW_CONFIG",
		"LISTINGVIEW_CONFIG_DIR",
		"LISTINGVIEW_DATA_DIR",
		"LISTINGVIEW_STATE_DIR",
		"LISTINGVIEW_CACHE_DIR",
		"LISTINGVIEW_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
	...
	return home, filepath.Join(home, ".config", "listingview-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "listingview-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("LISTINGVIEW_COOKIES", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "listingview-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "listingview-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "listingview-pp-cli", "credentials.toml")
