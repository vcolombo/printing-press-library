...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("audit_a_listing_s_seo_and_tags",
			mcplib.WithDescription("SEO score plus a per-tag grade with replacement suggestions."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Override the recipe's positional id value.")),
		),
		handleAuditAListingSSeoAndTags,
	)
}

// handleAuditAListingSSeoAndTags runs the audit_a_listing_s_seo_and_tags recipe intent: SEO score plus a per-tag grade with replacement suggestions.
func handleAuditAListingSSeoAndTags(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
