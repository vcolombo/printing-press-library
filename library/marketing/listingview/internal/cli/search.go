...
In local mode: searches locally synced data only.`,
  ...
  listingview-pp-cli search "error timeout"

  # Force local search only
  listingview-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  listingview-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  listingview-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /api/proxy/api/integration/etsy/getFilteredTags
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/api/proxy/api/integration/etsy/getFilteredTags", map[string]any{
					"search":      query,
					"sort_column": "opportunity",
					"sort_order":  "desc",
					"page":        1,
					"limit":       50,
				})
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'listingview-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
