...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Listingview",
...
Use --chrome to read cookies from Chrome for .app.listingview.io.
...
profile by name when the installed backend supports it.`,
		Example: `  listingview-pp-cli auth login --chrome
  listingview-pp-cli auth login --browser
  listingview-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  listingview-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  listingview-pp-cli auth login --browser\n")
				return nil
			}
			// Check if already authenticated via env var
			if v := os.Getenv("LISTINGVIEW_COOKIES"); v != "" {
				fmt.Fprintf(cmd.OutOrStdout(), "Already authenticated via %s env var.\n", "LISTINGVIEW_COOKIES")
				return nil
			}

			w := cmd.OutOrStdout()
			domain := ".app.listingview.io"
...
func newAuthRefreshCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  listingview-pp-cli auth refresh",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
func browserSessionTargetURL(cfg *config.Config) string {
	baseURL := strings.TrimRight("https://app.listingview.io", "/")
	if cfg != nil && cfg.BaseURL != "" {
		baseURL = strings.TrimRight(cfg.BaseURL, "/")
	}
	validationPath := strings.TrimSpace("/api/auth/me")
	if validationPath == "" {
		return baseURL
	}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  listingview-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  listingview-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: .app.listingview.io\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  listingview-pp-cli auth set-token eyJhbGciOi...",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  listingview-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if os.Getenv("LISTINGVIEW_COOKIES") != "" {
...
func browserSessionProofStatusForAuth(cfg *config.Config, authHeader string) (bool, string) {
	...
	if err != nil {
		return false, "proof not found; run listingview-pp-cli auth login --chrome"
	}
	var proof browserSessionProof
	if err := json.Unmarshal(data, &proof); err != nil {
		return false, "proof is not valid JSON; re-run listingview-pp-cli auth login --chrome"
	}
	if proof.APIName != "listingview" || proof.CookieDomain != ".app.listingview.io" || proof.ValidationPath != "/api/auth/me" {
...
func validateAndWriteBrowserSessionProof(ctx context.Context, cfg *config.Config, flags *rootFlags) error {
	...
	proof := browserSessionProof{
		APIName:               "listingview",
		CookieDomain:          ".app.listingview.io",
