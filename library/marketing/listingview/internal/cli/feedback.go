...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LISTINGVIEW_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LISTINGVIEW_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "listingview-pp-cli/feedback")
...
When ` + "`LISTINGVIEW_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LISTINGVIEW_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  listingview-pp-cli feedback list
  listingview-pp-cli feedback list --limit 5
  listingview-pp-cli feedback list --json`,
