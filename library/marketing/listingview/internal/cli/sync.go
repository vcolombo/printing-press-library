...
Resource scoping:
  ...
  listingview-pp-cli sync

  # Sync specific resources only
  listingview-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  listingview-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  listingview-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  listingview-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  listingview-pp-cli sync --latest-only`,
...
func syncResource(ctx context.Context, c interface {
	...
	for {
		...
		// ListingView: some list endpoints require a discriminator query param
		// the generic syncer doesn't supply (e.g. watchlist/list-favourite needs
		// type=). Inject it so sync doesn't 400. (Hand-fix in
		// listingview_research.go; re-apply if regenerated.)
		for k, v := range listingViewSyncRequiredParams(resource) {
			params[k] = v
		}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"watchlist": "/api/proxy/api/integration/etsy/list-favourite",
	}
