...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  listingview-pp-cli doctor
  listingview-pp-cli doctor --json
  listingview-pp-cli doctor --fail-on warn
  listingview-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "listingview-pp-cli auth login --chrome"
				} else {
					authConfigured = true
					report["auth"] = "configured (browser session)"
					report["auth_source"] = cfg.AuthSource
					report["auth_domain"] = ".app.listingview.io"
			...
			if os.Getenv("LISTINGVIEW_COOKIES") != "" {
				authEnvSet = append(authEnvSet, "LISTINGVIEW_COOKIES")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "LISTINGVIEW_COOKIES")
			}
			switch {
...
func browserSessionProofStatus(cfg *config.Config, authHeader string) (bool, string) {
	...
	if proof.APIName != "listingview" {
		return false, "proof belongs to a different CLI"
	}
	if proof.CookieDomain != ".app.listingview.io" {
		return false, "proof belongs to a different cookie domain"
	}
	if proof.ValidationPath != "/api/auth/me" {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'listingview-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'listingview-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'listingview-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'listingview-pp-cli sync' to refresh."
	}
	return report
}
