# Placeit Printed CLI Agent Guide

This directory is a generated `placeit-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
placeit-pp-cli doctor --json
placeit-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
placeit-pp-cli which "<capability>" --json
placeit-pp-cli <command> --help
```
...
```bash
placeit-pp-cli <command> --help
placeit-pp-cli <command> --dry-run --agent
```
