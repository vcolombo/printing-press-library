...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Placeit",
...
Use --chrome to read cookies from Chrome for placeit.net.
...
profile by name when the installed backend supports it.`,
		Example: `  placeit-pp-cli auth login --chrome
  placeit-pp-cli auth login --browser
  placeit-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  placeit-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  placeit-pp-cli auth login --browser\n")
				return nil
			}
			...
			requiredCookies := []string{"logged_session"}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  placeit-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  placeit-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: placeit.net\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  placeit-pp-cli auth set-token eyJhbGciOi...",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  placeit-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
func extractViaPycookiecheat(tool cookieTool, domain, profileDir string) (string, error) {
	...
	// Pass the domain and cookie-file path to Python through the process
	// environment rather than interpolating them into the -c source.
	// filepath.ToSlash does not escape double quotes, so a Chrome data dir
	// containing a `"` could otherwise terminate the Python string literal
	// early (or, in the worst case, inject code). os.environ values never
	// pass through a shell, so no escaping is required.
	var script string
	env := append(os.Environ(), "PP_COOKIE_DOMAIN="+cleanDomain)
	if cookiePath != "" {
		// Forward slashes keep Python from treating backslashes as escapes on Windows.
		env = append(env, "PP_COOKIE_FILE="+filepath.ToSlash(cookiePath))
		script = `import os, json; from pycookiecheat import chrome_cookies; print(json.dumps(chrome_cookies("https://" + os.environ["PP_COOKIE_DOMAIN"], cookie_file=os.environ["PP_COOKIE_FILE"])))`
	} else {
		script = `import os, json; from pycookiecheat import chrome_cookies; print(json.dumps(chrome_cookies("https://" + os.environ["PP_COOKIE_DOMAIN"])))`
	}
