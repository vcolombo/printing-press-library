...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "placeit-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PLACEIT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PLACEIT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "placeit-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/placeit-pp-cli/feedback.jsonl.
When ` + "`PLACEIT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PLACEIT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  placeit-pp-cli feedback list
  placeit-pp-cli feedback list --limit 5
  placeit-pp-cli feedback list --json`,
