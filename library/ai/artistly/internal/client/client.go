...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "artistly-pp-cli", "http")
	...
	// CheckRedirect: auth for this CLI rides on the cookie jar, which Go's
	// http.Client carries across redirect hops natively, so no Authorization
	// or Cookie header is re-derived here. The only customization is keeping
	// the 10-hop cap return a plain error (see below) rather than letting a
	// redirect loop fall through.
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "artistly-pp-cli auth set-token <token>")
}
