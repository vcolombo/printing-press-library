...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://app.artistly.ai",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("ARTISTLY_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "artistly-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// ARTISTLY_SESSION_COOKIE (headless/CI auth) counts as a configured
	// credential so `doctor` and `auth status` report accurately. It also seeds
	// the cookie jar that actually authenticates requests (see seedSessionFromEnv
	// in the cli package); here it only populates the in-memory "configured"
	// signal and is never written back to disk. Mirrors how `auth login --chrome`
	// stores its cookie blob in AccessToken.
	if v := strings.TrimSpace(os.Getenv("ARTISTLY_SESSION_COOKIE")); v != "" && cfg.AccessToken == "" && cfg.AuthHeaderVal == "" {
		cfg.AccessToken = "artistly_session=" + strings.TrimPrefix(v, "artistly_session=")
		cfg.AuthSource = "env"
	}
	...
	if v := os.Getenv("ARTISTLY_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
