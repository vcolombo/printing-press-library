...
Resource scoping:
  ...
  artistly-pp-cli sync

  # Sync specific resources only
  artistly-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  artistly-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  artistly-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  artistly-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  artistly-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/artistly-pp-cli/data.db)")
...
func syncResource(ctx context.Context, c interface {
	...
	for {
		...
		// PATCH(amend-2026-06-17: explicit records-path for designs).
		// Artistly's design endpoints return
		// {"results":{"designs":[...],"folders":[...],"hasMore":bool}}. The
		// generic extractor can't disambiguate the two sibling arrays
		// (extractSingleObjectArraySibling bails when arrayCount != 1), so it
		// fell back to the single-object path and stored the wrapper with no
		// id ("missing id for designs"). Result: 0 designs cached and the
		// headline offline-search feature was dead. A deterministic
		// resource->records-path map unwraps results.designs[] before the
		// heuristic result is trusted. Supersede once the generator profiles
		// nested multi-array list envelopes (see .printing-press-patches).
		if path, ok := resourceRecordsPath(resource); ok {
			if pathItems, ok := extractItemsByRecordsPath(data, path); ok {
				items = pathItems
				if c, more := extractDesignsPagination(data); more {
					nextCursor, hasMore = c, true
				} else {
...
// resourceRecordsPath returns the explicit nested key chain to the record
// array for resources whose envelope the generic heuristic can't resolve.
// PATCH(amend-2026-06-17): Artistly nests the design list under
// results.designs[] alongside a sibling results.folders[] array, which defeats
// the single-array-sibling fallback. Both design sync resources share the same
// {"results":{"designs":[...],"folders":[...],"hasMore":bool}} envelope.
func resourceRecordsPath(resource string) ([]string, bool) {
	paths := map[string][]string{
		"designs":                        {"designs"},
		"designs-fetch-personal-designs": {"designs"},
	}
	p, ok := paths[resource]
	return p, ok
}

// extractItemsByRecordsPath walks the explicit key chain in data and returns
// the terminal array as a slice of raw JSON objects. It returns ok=false (so
// the caller keeps the generic-extractor result) if any segment is missing or
// the terminus is not a non-empty array of objects.
func extractItemsByRecordsPath(data json.RawMessage, path []string) ([]json.RawMessage, bool) {
	cur := data
	...
	return extractObjectArray(cur)
}

// extractDesignsPagination reads the top-level hasMore flag from the Artistly
// design envelope (the client has already stripped the outer results wrapper by
// the time the sync loop sees the body). The endpoint carries no body cursor,
// so the page-int fallback in syncResource advances numerically when hasMore is
// true.
func extractDesignsPagination(data json.RawMessage) (string, bool) {
	var obj map[string]json.RawMessage
...
func defaultSyncResources() []string {
	return []string{
		"designs",
		"designs-fetch-personal-designs",
	}
}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"designs":                        "/designs-by-folder",
		"designs-fetch-personal-designs": "/fetch-personal-designs",
	}
