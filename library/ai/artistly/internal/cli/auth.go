...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Artistly",
...
func requiredAuthCookies() []string {
	raw := []string{"artistly_session", "XSRF-TOKEN"}
	out := raw[:0]
...
Use --chrome to read cookies from Chrome for app.artistly.ai.
...
profile by name when the installed backend supports it.`,
		Example: `  artistly-pp-cli auth login --chrome
  artistly-pp-cli auth login --browser
  artistly-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  artistly-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  artistly-pp-cli auth login --browser\n")
				return nil
			}

			w := cmd.OutOrStdout()
			domain := "app.artistly.ai"
			...
			requiredCookies := []string{"artistly_session", "XSRF-TOKEN"}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  artistly-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  artistly-pp-cli auth set-token eyJhbGciOi...",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  artistly-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
