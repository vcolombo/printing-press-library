...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'artistly-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API credentials."+
			"\n      See API docs: https://app.artistly.ai"+
			"\n      Run 'artistly-pp-cli doctor' to check auth status.", err))
...
func wrapWithProvenance(data json.RawMessage, prov DataProvenance) (json.RawMessage, error) {
	...
	if json.Valid(data) {
		...
		// A non-JSON body here is not data — most often the HTML login page an
		// expired session serves with HTTP 200. Never stringify it into results
		// (that ships HTML as data with a success exit); surface a clean error.
		// PATCH(artistly-provenance-no-stringify-nonjson): see .printing-press-patches/.
		if trimmed := bytes.TrimSpace(data); len(trimmed) > 0 && trimmed[0] == '<' {
			return nil, authErr(fmt.Errorf("not authenticated or session expired; run: artistly-pp-cli auth login --chrome"))
		}
		return nil, apiErr(fmt.Errorf("response was not valid JSON"))
	}
