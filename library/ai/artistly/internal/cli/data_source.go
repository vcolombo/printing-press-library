...
// assertLiveJSONBody guards a live JSON read. The Artistly app serves its HTML
// login page with HTTP 200 on an expired session; without this, that body would
// be handed back as data ("results") with a success exit (the silent-failure the
// generic read path otherwise allows). Detect the leading '<' — matching the
// guards in parseDesigns/fetchCheckpoints — and return a clean auth error.
//
// PATCH(artistly-live-read-html-auth-guard): see .printing-press-patches/.
func assertLiveJSONBody(data json.RawMessage) error {
	if trimmed := bytes.TrimSpace(data); len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; run: artistly-pp-cli auth login --chrome"))
	}
	return nil
}
...
func resolveReadWithStrategy(ctx context.Context, c *client.Client, flags *rootFlags, strategy string, resourceType string, isList bool, path string, params map[string]string, headers map[string]string, hintWriter io.Writer) (json.RawMessage, DataProvenance, error) {
	...
	if strategy == "live" {
		...
		if guardErr := assertLiveJSONBody(data); guardErr != nil {
			return nil, DataProvenance{}, guardErr
		}
	...
	default: // "auto"
		...
		if fallbackErr != nil {
			return nil, DataProvenance{}, fmt.Errorf("API unreachable and no local data. Run 'artistly-pp-cli sync' to enable offline access.\n\nOriginal error: %w", err)
		}
...
func writeThroughCache(ctx context.Context, resourceType string, data json.RawMessage) {
	rows := writeThroughCacheRows(resourceType, data)
	if len(rows) == 0 {
		return
	}
	db, err := store.OpenWithContext(ctx, defaultDBPath("artistly-pp-cli"))
	if err != nil {
		return
	}
	defer db.Close()

	_, _, _ = db.UpsertBatch(resourceType, rows)
}

// writeThroughCacheRows resolves the resource rows to upsert from a live API
// response body for the opportunistic write-through cache. It returns nil when
// the body carries nothing cacheable (an empty matched list envelope, an empty
// or unparseable body). Pure (no DB) so the shape-resolution is unit-testable.
//
// Resources that declare an explicit nested records-path (see
// resourceRecordsPath) are unwrapped deterministically before the generic
// envelope heuristics run. Without this, a multi-sibling-array list envelope
// like Artistly's {"designs":[...],"folders":[...],"hasMore":bool} defeats the
// single-array-sibling heuristic, falls through to the single-object branch, and
// caches the whole wrapper (no extractable id) instead of the records — the same
// defect the sync path fixed in resourceRecordsPath. (See .printing-press-patches.)
func writeThroughCacheRows(resourceType string, data json.RawMessage) []json.RawMessage {
	if path, ok := resourceRecordsPath(resourceType); ok {
		if items, isArray := arrayAtRecordsPath(data, path); isArray {
			// Declared list envelope for this resource: cache the records
			// (an empty array → nothing). Never fall through to the
			// single-object branch, which would cache the whole wrapper.
			return items
		}
		// Records-path key absent (e.g. a single-detail response): fall
		// through to the generic detail/list heuristics below.
	}
	...
	if json.Unmarshal(data, &envelope) != nil || len(envelope) == 0 {
		return nil
	}
	if extracted, ok := extractWriteThroughListItems(envelope); ok {
		// Matched a list envelope (possibly empty → nothing to cache).
		return extracted
	}
	...
	for _, key := range writeThroughListWrapperKeys {
		...
		// json.Unmarshal("null", &arr) succeeds with arr=nil, so require
		// arr != nil to keep true empty arrays in the skip branch while
		// letting scalar/null values fall through as regular field-name
		// collisions.
	...
	if hasListWrapperArray {
		for k := range envelope {
			if !listEnvelopeMetadataKeys[k] {
				// A real data field alongside the wrapper → single detail row.
				return []json.RawMessage{data}
			}
		}
		// Pure (empty) list envelope shape — nothing to cache.
		return nil
	}
	return []json.RawMessage{data}
}

// arrayAtRecordsPath walks the nested key chain in data and returns the terminal
// value as a slice of raw messages when it is a JSON array (empty or not).
// isArray is false when any segment is missing or the terminus is not an array,
// so the caller can fall back to the generic single-detail/list heuristics.
// Unlike extractItemsByRecordsPath (which requires a non-empty object array for
// the sync extractor), this treats an empty array as a valid empty list envelope
// so the write-through cache never falls through to caching the wrapper.
func arrayAtRecordsPath(data json.RawMessage, path []string) (items []json.RawMessage, isArray bool) {
	cur := data
	...
	if json.Unmarshal(cur, &items) != nil {
		return nil, false
	}
	return items, true
}
...
func resolveLocal(ctx context.Context, flags *rootFlags, hintWriter io.Writer, resourceType string, isList bool, path string, params map[string]string, reason string) (json.RawMessage, DataProvenance, error) {
	db, err := openStoreForRead(ctx, "artistly-pp-cli")
	if err != nil {
		return nil, DataProvenance{}, fmt.Errorf("opening local database: %w\nRun 'artistly-pp-cli sync' first.", err)
	}
	if db == nil {
		return nil, DataProvenance{}, fmt.Errorf("no local data. Run 'artistly-pp-cli sync' first")
	}
	defer db.Close()
	...
	if isList {
		...
		if len(items) == 0 {
			return nil, DataProvenance{}, fmt.Errorf("no local data for %q. Run 'artistly-pp-cli sync' first", resourceType)
		}
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, DataProvenance{}, fmt.Errorf("resource %q with ID %q not found in local store. Run 'artistly-pp-cli sync' first", resourceType, id)
		}
		return nil, DataProvenance{}, fmt.Errorf("querying local store: %w", err)
	}
