...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "artistly-pp-cli",
		Short: `Artistly CLI — The first CLI for Artistly — scriptable AI image generation, batch runs, and a searchable local archive the web app has…`,
		Long: `Artistly CLI — The first CLI for Artistly — scriptable AI image generation, batch runs, and a searchable local archive the web app has…
...
Health check: run 'artistly-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("artistly-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'artistly-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCheckpointsCmd(flags))
	...
	rootCmd.AddCommand(newNovelGenerateCmd(flags))
	rootCmd.AddCommand(newNovelPresetCmd(flags))
	rootCmd.AddCommand(newNovelQuotaCmd(flags))
	rootCmd.AddCommand(newNovelRedoCmd(flags))
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	// Headless/CI auth: ARTISTLY_SESSION_COOKIE seeds the jar before the client
	// loads it, so no `auth login --chrome` is required.
	seedSessionFromEnv(cfg.BaseURL)
