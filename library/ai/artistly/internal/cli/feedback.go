...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ARTISTLY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ARTISTLY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "artistly-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/artistly-pp-cli/feedback.jsonl.
When ` + "`ARTISTLY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ARTISTLY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  artistly-pp-cli feedback list
  artistly-pp-cli feedback list --limit 5
  artistly-pp-cli feedback list --json`,
