...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  artistly-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  artistly-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"designs", "designs-fetch-personal-designs"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  artistly-pp-cli workflow status

  # Show status as JSON
  artistly-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("artistly-pp-cli")
			}
