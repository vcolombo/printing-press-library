# Artistly Printed CLI Agent Guide

This directory is a generated `artistly-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
artistly-pp-cli doctor --json
artistly-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
artistly-pp-cli which "<capability>" --json
artistly-pp-cli <command> --help
```
...
```bash
artistly-pp-cli <command> --help
artistly-pp-cli <command> --dry-run --agent
```
