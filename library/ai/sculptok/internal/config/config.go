...
type Config struct {
	...
	SculptokApiKey string            `toml:"api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.sculptok.com/api-open",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("SCULPTOK_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "sculptok-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("SCULPTOK_API_KEY"); v != "" {
		cfg.SculptokApiKey = v
		cfg.markEnvOverride("SculptokApiKey")
		cfg.AuthSource = "env:SCULPTOK_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.SculptokApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("SCULPTOK_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	replacements := map[string]string{
		"token":            token,
		"api_key":          c.SculptokApiKey,
		"SCULPTOK_API_KEY": c.SculptokApiKey,
	}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.SculptokApiKey = token
	delete(c.envOverrides, "SculptokApiKey")
	c.updateFileConfigField("SculptokApiKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["SculptokApiKey"] {
			out.SculptokApiKey = c.fileConfig.SculptokApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "SculptokApiKey":
		c.fileConfig.SculptokApiKey = c.SculptokApiKey
	}
}
