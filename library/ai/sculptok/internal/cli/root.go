...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "sculptok-pp-cli",
		Short: `SculptOK CLI — turn a local image into a depth map, printable STL, or 3D model in one command, with credit-cost preflight and an offline job store`,
		Long: `SculptOK CLI — the first CLI and MCP for SculptOK: turn a local image into a depth map, printable STL, or 3D model in one command, with credit-cost preflight and a local job store no other tool has.
...
Health check: run 'sculptok-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("sculptok-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'sculptok-pp-cli profile list')")
	...
	rootCmd.AddCommand(newDrawCmd(flags))
	...
	rootCmd.AddCommand(newDrawingsPromotedCmd(flags))
