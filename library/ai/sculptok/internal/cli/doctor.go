...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sculptok-pp-cli doctor
  sculptok-pp-cli doctor --json
  sculptok-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export SCULPTOK_API_KEY=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("SCULPTOK_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "SCULPTOK_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SCULPTOK_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "sculptok-pp-cli", suggestion)
						} else {
