...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "sculptok-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SCULPTOK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SCULPTOK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "sculptok-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/sculptok-pp-cli/feedback.jsonl.
When ` + "`SCULPTOK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SCULPTOK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sculptok-pp-cli feedback list
  sculptok-pp-cli feedback list --limit 5
  sculptok-pp-cli feedback list --json`,
