# SculptOK Printed CLI Agent Guide

This directory is a generated `sculptok-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
sculptok-pp-cli doctor --json
sculptok-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
sculptok-pp-cli which "<capability>" --json
sculptok-pp-cli <command> --help
```
...
```bash
sculptok-pp-cli <command> --help
sculptok-pp-cli <command> --dry-run --agent
```
