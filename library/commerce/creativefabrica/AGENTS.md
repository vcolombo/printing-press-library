# Creative Fabrica Printed CLI Agent Guide

This directory is a generated `creativefabrica-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
creativefabrica-pp-cli doctor --json
creativefabrica-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
creativefabrica-pp-cli which "<capability>" --json
creativefabrica-pp-cli <command> --help
```
...
```bash
creativefabrica-pp-cli <command> --agent
```
...
```bash
creativefabrica-pp-cli <command> --help
creativefabrica-pp-cli <command> --dry-run --agent
```
