...
'deals'; for a designer's whole catalog use 'designer'.`,
  ...
  creativefabrica-pp-cli find "logo" --max-price 2 --on-sale --json --select name,price,url`, "\n"),
	...
	f := cmd.Flags()
	f.StringVar(&itemType, "type", "", "Product type (Graphics, Fonts, Crafts, Embroidery, Laser Cutting, 3D SVG, Bundles, ...)")
	f.StringVar(&category, "category", "", "Category facet (see 'categories'), e.g. Icons, Patterns, T-shirt Designs")
	f.StringVar(&designer, "designer", "", "Restrict to a designer (numeric id or exact name)")
	f.StringVar(&format, "format", "", "File format(s), comma-separated: svg,dxf,png,eps,pes (matched in tags/titles)")
	f.BoolVar(&pod, "pod", false, "Only print-on-demand / commercial-license assets")
	f.BoolVar(&free, "free", false, "Only free assets")
	f.BoolVar(&onSale, "on-sale", false, "Only assets currently on promotion")
	f.BoolVar(&noSub, "no-subscription", false, "Only assets usable without an active subscription")
	f.Float64Var(&maxPrice, "max-price", 0, "Maximum price (0 = no limit)")
	f.StringVar(&sortBy, "sort", "relevance", "Sort order: relevance | newest")
	f.IntVar(&page, "page", 0, "Result page (0-based)")
	f.IntVar(&limit, "limit", 20, "Max results to return")
	return cmd
}
