...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "creativefabrica-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CREATIVEFABRICA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CREATIVEFABRICA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "creativefabrica-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/creativefabrica-pp-cli/feedback.jsonl.
When ` + "`CREATIVEFABRICA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CREATIVEFABRICA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  creativefabrica-pp-cli feedback list
  creativefabrica-pp-cli feedback list --limit 5
  creativefabrica-pp-cli feedback list --json`,
